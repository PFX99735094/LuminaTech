...
import { Cpu, Eye, EyeOff, Mail, Sparkles, UserPlus } from 'lucide-react';
...
export function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    setLoading(true);
    const { error: err } = await signUp(name, email, password, school || undefined);
    if (err) {
      setError(err);
    } else {
      setSuccess(true);
    ...
    return (
      <div className="min-h-screen bg-paper-50 font-body text-ink-900 selection:bg-cyan-spark selection:text-ink-900">
        ...
        <main className="mx-auto flex w-full max-w-lg flex-col items-center px-6 py-24 text-center">
          <span className="grid h-16 w-16 place-items-center rounded-full border-2 border-ink-900 bg-lime-spark">
            <Sparkles className="h-8 w-8" strokeWidth={2.25} />
          </span>
          <h1 className="mt-6 font-display text-3xl font-black text-ink-900">Conta criada!</h1>
          ...
          <Link
            to="/login"
            className="mt-8 inline-flex items-center gap-2 rounded-md border-2 border-ink-900 bg-violet-deep px-5 py-3 font-mono text-[11px] font-bold uppercase tracking-[0.18em] text-paper-50 shadow-[3px_3px_0_0_#4C1D95] transition-all hover:translate-x-[2px] hover:translate-y-[2px] hover:bg-cyan-spark hover:text-ink-900 hover:shadow-[1px_1px_0_0_#4C1D95]"
          >
            Ir para o login
          </Link>
        </main>
    ...
    <div className="min-h-screen bg-paper-50 font-body text-ink-900 selection:bg-cyan-spark selection:text-ink-900">
      ...
      <main className="mx-auto flex w-full max-w-md flex-col px-6 py-16 lg:py-24">
        <div className="text-center">
          <h1 className="font-display text-4xl font-black text-ink-900">Criar Conta</h1>
        ...
        <form onSubmit={handleSubmit} className="mt-10 space-y-5">
          ...
          <label className="flex flex-col gap-1.5">
            <span className="font-mono text-[10.5px] font-bold uppercase tracking-[0.18em] text-ink-900/70">Nome completo</span>
            <input
              type="text"
              value={name}
          ...
          <label className="flex flex-col gap-1.5">
            <span className="font-mono text-[10.5px] font-bold uppercase tracking-[0.18em] text-ink-900/70">
              Rede de Ensino / Escola <span className="font-normal text-ink-900/45">(opcional)</span>
            </span>
            <input
              type="text"
              value={school}
              ...
              className="w-full rounded-md border-2 border-ink-900 bg-paper-50 px-3 py-2.5 font-body text-[15px] text-ink-900 placeholder:text-ink-900/45 focus:shadow-[2px_2px_0_0_#4C1D95] focus:outline-none"
              placeholder="Ex: EMEF Professor Amâncio"
            />
          </label>
        ...
        <p className="mt-8 text-center font-mono text-[11px] text-ink-900/60">
          Já tem conta?{' '}
          <Link to="/login" className="font-bold text-cyan-spark underline decoration-2 underline-offset-4 hover:text-cyan-deep">
            Entrar
          </Link>
        </p>
      </main>
